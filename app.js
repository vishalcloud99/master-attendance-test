...
function handleRouteChange() {
    ...
    if (hash === 'service-attendance') {
        setupCamera('sa-video');
        setupCamera('lb-video'); // Attempt to start both if visible
    }
...
async function setupCamera(videoId) {
    ...
    try {
        ...
        alert("Camera access is required for attendance tracking. Please allow permissions.");
    }
}
...
async function handleGeneralAttendance(e) {
    ...
    try {
        ...
        if (!base64Data) {
            Utils.hideLoader();
            return alert("Please take a live selfie first.");
        }
...
async function handleSalesVisit(e) {
    ...
    try {
        ...
        if (res.status === 'success') {
            alert('Visit Successfully Submitted!');
...
async function handleServiceAttendance(e) {
    ...
    try {
        ...
        if (numLabour > 0) {
            ...
            if (!labourData) {
                Utils.hideLoader();
                return alert("Please capture the live labour photo first.");
            }
        }
        ...
        if (res.status === 'success') {
            alert('Service Attendance Successfully Submitted!');
